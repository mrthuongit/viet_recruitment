'use strict';
angular.module('employerModule').filter('category', function(_) {
...
.filter('existQuestion', function(_) {
 ...
 .filter('singleChoice', function(_) {
    return function(questionList) {
        return _.filter(questionList, function(question)
        {
            return question.type && question.type ==='SC';
        });
    };
  })
.controller('QuestionQuizInterviewController', function($scope, $employer, $rootScope, localStorageService, $location, $common, $q,$cache,_,toastr,$translate)
{
    var alert = $translate.instant('toarster.alert');
    var optionchoose = $translate.instant('employer.optionchoose');
    ...
    function addQuestionSystem(questionId) {
        ...
        $scope.interviewQuestion.options = question.options;
    ...
    $scope.moveDownInterviewQuestion = function(interviewQuestion)
    {
        ...
        var sortQuestionList = _.sortBy($scope.interviewQuestionList, 'order');
        $scope.interviewQuestionList = sortQuestionList;
        localStorageService.set("interviewQuestionList", sortQuestionList);
    };
    ...
    $scope.addQuestionManual = function()
    {
        ...
        var optiontrue = _.find($scope.interviewQuestion.options, function(item){
            if(item.correct){
                return true;
            }
        });
        if(optiontrue){
            ...
            toastr.warning( optionchoose,alert);
        }
    };

    $scope.addOption = function() {
        if (!$scope.newOption.title.trim()) return;
        var itemoption = _.find($scope.interviewQuestion.options, function(item){
            return item.title == $scope.newOption.title;
        });
        if(!itemoption) {
            var option = {title:angular.copy($scope.newOption.title),correct:false};
            $scope.interviewQuestion.options.push(option);
            $scope.newOption.title = '';
        } else {
            $scope.newOption.title = '';
        }
    ...
    {
         localStorageService.set('interview', $scope.interview);
         $location.path("/employer/quiz/review_confirm_interview");
    };
    init();
});
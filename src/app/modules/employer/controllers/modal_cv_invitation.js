'use strict';
angular.module('employerModule').controller('ModalCvInvitationController', function($scope,$rootScope,$translate,localStorageService, $location, $common, $q, blockUI, $employee,$uibModal,$uibModalInstance, $employer, data, _, toastr) {
    ...
    $scope.avatar = '' ;
    $scope.eduLevelsList = [];
    $scope.chooseEmployee = data.candidate;
    $scope.employer = localStorageService.get('userEmployer');
    // localStorageService.remove('employeecv');
    ...
    $scope.getEmployeeProfile = function() {
        var promise =  new Promise(function(resolve, reject) {
            var employee = $scope.chooseEmployee.profile;
            ...
            if (employee.gender) {
                $scope.genders.selectedGender = _.find($scope.genders.availableGenders, function(gender) {
            ...
            // Update firstName
            // localStorageService.set('employeecv', employee);
    ...
    $scope.getEmployeeExp = function() {
        var promise = new Promise(function(resolve, reject) {
            ...
            $scope.expList = $scope.chooseEmployee.expList;
    ...
    $scope.getEduLevelsList = function() {
        var promise = new Promise(function(resolve, reject) {
            ...
            if (localStorageService.get('eduLevelsList')) {
                ...
                $common.getEducationLevel({ lang: 'vi' }, function(result) {
                    if (result.status) {
    ...
    $scope.getEmployeeEdu = function() {
        var promise = new Promise(function(resolve, reject) {
            localStorageService.set('eduList', $scope.chooseEmployee.eduList);
            $scope.eduList =$scope.chooseEmployee.eduList;
                    // Get name edu level
    ...
    $scope.getEmployeeCer = function() {
        var promise = new Promise(function(resolve, reject) {
            $scope.certList = $scope.chooseEmployee.certList;
            resolve('done');
        });
        return promise;
    };
    ...
    $scope.getEmployeeDoc = function() {
        var promise = new Promise(function(resolve, reject) {
            $scope.docList = $scope.chooseEmployee.docList;
            resolve('done');
        });
    ...
    $scope.displayContact = function() {
        var info = {
            ...
            employeeId: $scope.chooseEmployee.employeeId
        };

        $employer.getEmployeeContact(info, function(result) {
            if (result.status) {
                $scope.chooseEmployee.viewed = result.data.result;
            }
        });
    };
    ...
    $scope.genders = {
        ...
        selectedGender: { id: 0, title: select }
    };

    var init = function() {
        // Inital Emplyee information
        if($scope.chooseEmployee.profile){
        ...
        }).then(function()
        {
            ...
            $scope.employee.email = $scope.chooseEmployee.email;
            localStorageService.set('employeedv', $scope.employee);
        }
    };
    init();
});